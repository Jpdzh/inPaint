...
export default class Pen extends Plugin {
  ...
  onDrawStart = (drawEventParams: DrawEventParams) => {
    ...
    this.lastLine = new Konva.Line({
      id: uuid(),
      stroke:
        paramValue && paramValue.color
          ? paramValue.color
          : this.defaultParamValue.color,
      strokeWidth:
        paramValue && paramValue.strokeWidth
          ? paramValue.strokeWidth
          : this.defaultParamValue.strokeWidth,
      globalCompositeOperation: "source-over",
      points: [pos.x, pos.y],
      dashEnabled: !!(
        paramValue &&
        paramValue.lineType &&
        paramValue.lineType === "dash"
      ),
      dash: [8],
      tension: 1,
      lineCap: "round",
